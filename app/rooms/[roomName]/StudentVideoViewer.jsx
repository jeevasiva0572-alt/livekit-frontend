'use client';
// Version: Fixed-Playback-1.2 (dead-track guard + drift fix)
...
export default function StudentVideoPanel({ isHero = false, ignoreLocal = false }) {
  ...
  // ✅ Track exists but can still be "ended" -> black screen
  const isTrackLive =
    !!presenterTrack?.publication?.track &&
    presenterTrack.publication.track.mediaStreamTrack?.readyState === 'live';
  ...
  useEffect(() => {
    console.log('📡 [StudentVideoViewer] Track Status:', {
      ...
      showTeacherVideo,
      isTrackLive,
      readyState: presenterTrack?.publication?.track?.mediaStreamTrack?.readyState,
    });
  }, [screenShareTracks.length, !!presenterTrack, isHero, showTeacherVideo, isTrackLive]);
      ...
      try {
        ...
        if (msg.action === 'VIDEO_TIME_UPDATE') {
          ...
          if (typeof msg.currentTime === 'number') {
            // ✅ use functional setState to avoid stale currentTime
            setCurrentTime(prev => {
              const drift = Math.abs(prev - msg.currentTime);
              return drift > 2 ? msg.currentTime : prev;
            });
          }
        }
  ...
  // force remount when track changes
      ...
      {isHero && (
        <div
          style={{
            ...
            color: isTrackLive ? '#81c995' : '#f28b82',
            fontSize: '10px',
            borderRadius: '4px',
            zIndex: 100,
        ...
        >
          Status: {isTrackLive ? 'Connected' : 'Waiting...'}
        </div>
      )}
